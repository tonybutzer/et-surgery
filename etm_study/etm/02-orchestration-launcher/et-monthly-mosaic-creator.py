...
# THIS is a work in progress - needs some help
...
class Etm:
    def __init__(self, yml_file='monthly_etm_run.yml'):
    ...
    def _start_etm(self, year, product, unmosaicked_input):
        #unmosaicked_input=self.etm_parms['unmosaicked_input']
        template=self.etm_parms['enduser_cog_output']
        tile = unmosaicked_input.split('/')[-2]
        enduser_cog_output = template.replace('TBD', tile)

        month_year = 'monthly'
        #month_year = 'monthly_'+'2009'
    
        cmd_opt = '-i ' + unmosaicked_input + ' -o ' + enduser_cog_output + ' -y ' + month_year + ' ' + product + ' dummy'
        print(cmd_opt)
        ...
        #print(docker_image)
        tile = enduser_cog_output.split('/')[-2]
        name_c = f'etmv1_{year}_{product}_{tile}'
    ...
    def _event_loop(self, year_to_process, end_year, product):
        ...
        # SINCE this does all years dont need the while
        # while year_to_process <= end_year:
        for item in  self.etm_parms['unmosaicked_inputs']:
            ...
            if (mem and cpu and containers):
                print("OK to Launch")
                unmosaicked_input = item
                self._start_etm('monthly-', product, unmosaicked_input) ### start mosaic container
    ...
    # ============================================================
        print(f"EVENT LOOP COMPLETE for - {product}")
